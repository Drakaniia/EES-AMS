...
use crate::sf2::sf2_metadata::{date_mappings_from_analysis, metadata_from_import_analysis};
use chrono::Utc;
...
fn import_computes_roster_name_marks_from_analysis() {
    ...
    let students = vec![make_student("1", "Juan", Some(StudentGender::Male))];
    let assignments = template_roster_assignments(&students).unwrap();
...
fn import_metadata_from_analysis_has_configure_calendar() {
    ...
    assert!(
        metadata.configure_calendar,
        "imported workbook should configure calendar"
    );
...
fn import_date_mappings_from_analysis_produces_correct_structure() {
    ...
    analysis.dates = vec![crate::sf2::models::Sf2WorkbookDate {
...
fn import_mapped_rows_and_occupied_set_match() {
    ...
    for &row in &mapped_rows {
        assert!(
            ...
            "occupied set must contain all mapped rows"
        );
    }
}
