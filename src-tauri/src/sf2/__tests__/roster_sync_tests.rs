...
fn roster_sync_formula_marks_computes_correct_marks_for_standard_roster() {
    ...
    assert_eq!(total_marks.len(), 6, "should have 6 total formula marks (2 dates × 3 rows)");

    // MALE TOTAL (F29): =15-COUNTIF(F8:F28,"X")
    let male_f = total_marks.iter().find(|m| m.cell_address == "F29").unwrap();
    assert_eq!(male_f.value, "=15-COUNTIF(F8:F28,\"X\")");
    assert_eq!(male_f.sheet_name, "JUNE 2026");

    // FEMALE TOTAL (F49): =10-COUNTIF(F30:F48,"X")
    let female_f = total_marks.iter().find(|m| m.cell_address == "F49").unwrap();
    assert_eq!(female_f.value, "=10-COUNTIF(F30:F48,\"X\")");
    assert_eq!(female_f.sheet_name, "JUNE 2026");

    // Combined TOTAL (F50): =F29+F49
    let combined_f = total_marks.iter().find(|m| m.cell_address == "F50").unwrap();
    ...
    let ar53 = summary_static_marks.iter().find(|m| m.cell_address == "AR53").unwrap();
    assert_eq!(ar53.value, "15");
    assert_eq!(ar53.sheet_name, "JUNE 2026");

    let as53 = summary_static_marks.iter().find(|m| m.cell_address == "AS53").unwrap();
    assert_eq!(as53.value, "10");
    assert_eq!(as53.sheet_name, "JUNE 2026");

    let at53 = summary_static_marks.iter().find(|m| m.cell_address == "AT53").unwrap();
    assert_eq!(at53.value, "25");
    assert_eq!(at53.sheet_name, "JUNE 2026");

    // Verify summary formula marks exist (12 per sheet: 4 rows × 3 cols)
    assert_eq!(summary_formula_marks.len(), 12, "should have 12 summary formula marks (4 rows × 3 cols)");

    // Row 63 ADA references correct total rows
    let ar63 = summary_formula_marks.iter().find(|m| m.cell_address == "AR63").unwrap();
    assert_eq!(ar63.value, "=IFERROR(AVERAGE(F29:AL29),0)", "male ADA should reference row 29");

    let as63 = summary_formula_marks.iter().find(|m| m.cell_address == "AS63").unwrap();
    assert_eq!(as63.value, "=IFERROR(AVERAGE(F49:AL49),0)", "female ADA should reference row 49");

    let at63 = summary_formula_marks.iter().find(|m| m.cell_address == "AT63").unwrap();
    assert_eq!(at63.value, "=IFERROR(AVERAGE(F50:AL50),0)", "combined ADA should reference row 50");
}
...
fn roster_sync_formula_marks_empty_date_mappings_returns_empty() {
    ...
    let (total_marks, summary_formula_marks, summary_static_marks) = roster_sync_formula_marks(
        15, 10, 29, 49, 50, &date_mappings,
    );

    assert!(total_marks.is_empty(), "no total marks when date_mappings is empty");
    assert!(summary_formula_marks.is_empty(), "no summary formula marks when date_mappings is empty");
    assert!(summary_static_marks.is_empty(), "no summary static marks when date_mappings is empty");
}
...
fn roster_sync_formula_marks_expanded_roster_uses_shifted_total_rows() {
    ...
    assert_eq!(total_marks.len(), 3, "should have 3 total marks (1 date × 3 rows)");

    // MALE TOTAL at P33 = =25-COUNTIF(P8:P32,"X")
    let male_mark = total_marks.iter().find(|m| m.cell_address == "P33").unwrap();
    assert_eq!(male_mark.value, "=25-COUNTIF(P8:P32,\"X\")");

    // FEMALE TOTAL at P56 = =22-COUNTIF(P34:P55,"X")
    let female_mark = total_marks.iter().find(|m| m.cell_address == "P56").unwrap();
