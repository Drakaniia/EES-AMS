...
pub fn worksheet_cell(sheet: &ComObject, row: i32, column: i32) -> Result<ComObject> {
    ...
    cells.get_object_with_args(
        "Item",
        vec![
            crate::sf2::excel::excel_com::com_session::ComVariant::i4(row),
            crate::sf2::excel::excel_com::com_session::ComVariant::i4(column),
        ],
    )
}
...
pub fn set_sf2_mark_force(sheet: &ComObject, cell_address: &str, value: &str) -> Result<()> {
    let cell = sheet.get_object_with_args(
        "Range",
        vec![crate::sf2::excel::excel_com::com_session::ComVariant::bstr(
            cell_address,
        )],
    )?;
    let target = merged_target(&cell)?;
...
pub fn merged_target(cell: &ComObject) -> Result<ComObject> {
    ...
    cells.get_object_with_args(
        "Item",
        vec![
            crate::sf2::excel::excel_com::com_session::ComVariant::i4(1),
            crate::sf2::excel::excel_com::com_session::ComVariant::i4(1),
        ],
    )
}
...
fn put_cell_value(cell: &ComObject, value: &str) -> Result<()> {
    if value.is_empty() {
        cell.put_variant(
            "Value2",
            crate::sf2::excel::excel_com::com_session::ComVariant::empty(),
        )
    } else {
...
fn put_cell_value_numeric(cell: &ComObject, value: &str) -> Result<()> {
    if value.is_empty() {
        ...
        cell.put_variant(
            "Value2",
            crate::sf2::excel::excel_com::com_session::ComVariant::i4(num),
        )
    } else {
        cell.put_string("Value2", value)
    }
