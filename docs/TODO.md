...
- Comprehensive full UI page: SF2 Workbook Summary Page UI before export. (i can see the details of the sf2 such as school name, id yar, advidser and schoolhead name )
- In Global settings card, when left unsaved, create a modal dialogue warning that you have unsaved changes, rather than letting the global config reset when out of focus.
- Create release.
- Create a manual in PDF.
- Morph animation when switching tabs in Add Student modal.
- Add progress bar and loading animation when importing new SF2 or creating SF2 workbook, instead of a hanging unclickable button (also include in end session closing).
