...
export function getPortalRedirectUrl(role: string, redirectUrl?: string): string {
  const portals = {
    ...
    faculty: process.env.NEXT_PUBLIC_FACULTY_URL || 'http://localhost:3002',
    admin: process.env.NEXT_PUBLIC_ADMIN_URL || 'http://localhost:3003',
    parent: process.env.NEXT_PUBLIC_PARENT_URL || 'http://localhost:3004',
  };
  ...
  // If there's a specific redirect URL that belongs to the correct portal, use it
  if (redirectUrl) {
    try {
      const redirect = new URL(redirectUrl);
      const portal = new URL(portalUrl);
      
      // Check if the redirect URL is from the correct portal
